...
func NewHTTPRequestHandler(s *server.HTTPServer, conn net.Conn, enableCGI bool) interface{} {
	if enableCGI {
		return NewCGIHTTPRequestHandler(s, conn)
	}
	return NewSimpleHTTPRequestHandler(s, conn)
}
